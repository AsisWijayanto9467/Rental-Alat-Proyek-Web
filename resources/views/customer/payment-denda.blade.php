...
@section('title', 'Payment Denda - RENTAL PRO')

@section('content')
<div class="bg-[#F5F5F3] py-10">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-2xl font-extrabold text-[#2A2A2A]">Bayar Denda</h1>
            <p class="text-gray-500 mt-1">Upload bukti pembayaran denda untuk penyewaan {{ $penyewaan->kode_penyewaan }}.</p>
        </div>

        {{-- Denda Summary --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            <h3 class="font-bold text-[#2A2A2A] mb-4">Ringkasan Denda</h3>
            <div class="space-y-3 text-sm">
                ...
                <div class="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span class="font-semibold text-[#2A2A2A] capitalize">{{ $denda->jenis_denda }}</span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-500">Alasan</span>
                    <span class="font-semibold text-[#2A2A2A]">{{ $denda->alasan }}</span>
                </div>
                <div class="flex justify-between py-3">
                    <span class="font-bold text-[#2A2A2A]">Total Denda</span>
                    <span class="font-extrabold text-red-600 text-lg">Rp {{ number_format($denda->jumlah, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('payment.store') }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="metode_pembayaran" value="cash" class="peer sr-only">
                            <div class="border-2 border-gray-200 peer-checked:border-[#F7C264] peer-checked:bg-[#FFF9EB] rounded-xl p-4 text-center transition hover:border-gray-300">
                ...
                <div>
                    ...
                    <textarea name="catatan" rows="2" class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-[#F7C264] focus:border-[#F7C264] transition resize-none">{{ old('catatan') }}</textarea>
                </div>
